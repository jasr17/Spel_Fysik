...
struct ShaderLight {
	...
	float4 smapSize = float4(0, 0, 0, 0);
};
struct ShaderLights {
...
public:
	...
	float2 getSmapResolution(int index);
	...
	void addLight(float3 position, float3 color, float intensity, float3 lookAt = float3(0, 0, 0), float FOV = XM_PI*0.45, float nearPlane = 0.01,
		float farPlane = 50, int smapWidth = SMAP_WIDTH, int smapHeight = SMAP_HEIGHT);
