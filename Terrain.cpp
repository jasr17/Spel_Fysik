...
void Terrain::freeGrid(Vertex ** grid)
{
...
void Terrain::convertGridToMesh_flatShading()
{
	mesh.reset();
...
void Terrain::convertGridToMesh_smoothShading()
{
	smoothNormals();
	mesh.reset();
	mesh.appendCapacity(tileCount.x*tileCount.y * 6);
...
void Terrain::applyHeightToGrid(const wchar_t* filePath)
{
	//get texture
...
void Terrain::smoothSurface(float s)
{
	//create temp grid
...
float3 Terrain::getTerrainSize() const
{
	return scale;
}
void Terrain::rotateY(float y)
{
	rotation.y += y;
}
float4x4 Terrain::getWorldMatrix()
{
...
float Terrain::getHeightOfTerrainFromCoordinates(float x, float y)
{
...
float3 Terrain::getPointOnTerrainFromCoordinates(float x, float z)
{
...
bool Terrain::create(XMINT2 _tileCount, float desiredMapSize, float height, const wchar_t* filePath, typeOfShading shading)
{
	bool check = true;
...
Terrain::Terrain(XMINT2 _tileCount, float _tileSize, float height, const wchar_t* filePath, typeOfShading shading)
{
